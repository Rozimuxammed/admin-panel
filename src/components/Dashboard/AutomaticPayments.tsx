...
import { Send, CreditCard, Users, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
export const AutomaticPayments: React.FC = () => {
  ...
  const processPayment = async () => {
    if (selectedUsers.length === 0 || paymentAmount <= 0) {
      alert('Please select users and enter a valid amount');
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (newPayment.status === 'completed') {
        alert(`Payment of $${paymentAmount} successfully sent to ${selectedUsers.length} users via InterKassa`);
      } else {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Automatic Payments</h2>
        <p className="text-gray-600">Send payments to users automatically via InterKassa API</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="p-6 space-y-4">
            ...
            {selectedUsers.length > 0 && (
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                ...
                <div className="text-sm text-blue-800 space-y-1">
                  <p>Recipients: {selectedUsers.length} users</p>
                  <p>Amount per user: ${paymentAmount.toFixed(2)}</p>
            ...
            <button
              ...
              disabled={isProcessing || selectedUsers.length === 0 || paymentAmount <= 0}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        {user.subscriptionPlan} • Balance: ${user.accountBalance.toFixed(2)}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="font-medium text-gray-900">
                        ${payment.amount.toFixed(2)} to {payment.users.length} users
                      </div>
                      <div className="text-sm text-gray-500">
                        {payment.type} • {new Date(payment.date).toLocaleString()}
                      </div>
                      {payment.note && (
